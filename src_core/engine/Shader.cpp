...
void PipelineState::loadFromTable(sol::table config) {
  ...
  if (computeShaderSourceOpt) {
    ...
    barrierBits = config.get_or("barrierBits", (gl::GLbitfield)gl::ALL_BARRIER_BITS);
  } else {
    // Draw shader
    shaderType = ShaderType::Draw;
    {
      ...
      blendStates.for_each([this](sol::object key, sol::object value) {
        ...
        if (bs.enabled) {
          ...
          bs.funcDstRGB = table.get_or("funcDstRGB", (gl::GLenum)gl::ONE_MINUS_SRC_ALPHA);
...

    {
      ...
      if (dss.stencilEnable) {
        dss.stencilFace = table.get_or("stencilFace", (gl::GLenum)gl::FRONT_AND_BACK);
