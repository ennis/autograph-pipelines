...
// Uniquely identifies a resource on the filesystem, on the network, in an archive file, or in memory
...
public:
	...
	{}
	ResourceID(const std::experimental::filesystem::path& p) : str_{ p.string() }, path_{p}
	{
	}
	...
	auto getFilesystemPathString() -> std::string {
		return path_.string();
	}
