...
namespace ag
{
		void CameraControl::zoomIn(float dzoom)
		{
			zoomLevel_ += dzoom;
		}

		void CameraControl::setZoom(float zoom)
		{
			zoomLevel_ = zoom;
		}
		...
		//
		void CameraControl::lookAt(vec3 lookAt)
		{
			target_ = lookAt;
		}
		...
		void CameraControl::lookDistance(float lookDist)
		{
			radius_ = lookDist;
		}
		...
		void CameraControl::setFieldOfView(float fov) {
			fov_ = fov;
		}
