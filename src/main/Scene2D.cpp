...
void Scene2D::loadTilemap(const char *id) {
  ...
  for (auto pair : tilesets) {
    ...
    if (tileWidth_ == 0)
      tileWidth_ = ts.tileWidth;
    if (tileHeight_ == 0)
      tileHeight_ = ts.tileHeight;
  ...
  for (auto pair : layers) {
    ...
    // create texture
    map.tileMapTex = gl::Texture::create2D(ImageFormat::R32G32_SFLOAT,
...
void Scene2D::render(gl::Framebuffer &screen, float viewX, float viewY,
...
void Scene2D::render(gl::Framebuffer &screen, const Viewport &viewport) {
  ...
  int screenHeightTiles =
      (int)(viewport.height + tileHeight_ - 1) / tileHeight_;
  // check if the current vertex grid is enough
  if (screenWidthTiles + 1 > gridWidthTiles_ ||
      screenHeightTiles + 1 > gridHeightTiles_) {
    // re-create the VBO
  ...
  auto pfmod = [](float x, float y) { return x - y * std::floor(x / y); };
  ...
  draw(screen, tileGrid_, tileMapShader_, texture(0, *tileset.tex, sampler_),
