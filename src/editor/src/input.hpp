...
namespace input {
enum class REFLECT button_state { pressed, released };
enum class REFLECT key_state { pressed, released, repeat };

enum class REFLECT input_event_type {
  mouse_button,
  mouse_move,
  cursor,
  mouse_scroll,
...
struct REFLECT mouse_button_event
...
struct REFLECT cursor_event : public t_input_event<input_event_type::cursor> {
  using ptr = std::shared_ptr<cursor_event>;
  // in client units (pixels)
  glm::ivec2 pos;
};

struct REFLECT mouse_move_event : public t_input_event<input_event_type::mouse_move> {
  using ptr = std::shared_ptr<mouse_move_event>;
  glm::ivec2 delta;
};

struct REFLECT mouse_scroll_event
    : public t_input_event<input_event_type::mouse_scroll> {
  using ptr = std::shared_ptr<mouse_scroll_event>;
  glm::ivec2 delta;
};

struct REFLECT key_event : public t_input_event<input_event_type::key> {
  using ptr = std::shared_ptr<key_event>;
  uint32_t code;
  key_state state;
};

struct REFLECT text_event : public t_input_event<input_event_type::text> {
	uint32_t codepoint;
};

struct REFLECT stylus_proximity_event
...
struct REFLECT stylus_properties_event
...
struct REFLECT gamepad_axis {
	glm::vec2 pos;
};

struct REFLECT gamepad_state {
...
class REFLECT action : public observable<> {
public:
  action() {}
  virtual ~action() {}
...
class REFLECT key_action : public action {
public:
  key_action(int key);
  ~key_action();
...
class REFLECT gamepad_button_action : public action {
public:
