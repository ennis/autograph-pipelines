...
{
	if (gApplication)
	gApplication->handleWindowSizeChanged(uvec2{ (unsigned)width, (unsigned)height });
}
...
Application::Application(uvec2 initSize) : impl{std::make_unique<Impl>()} {
  initialize(initSize);
}
...
uvec2 Application::getFramebufferSize() {
  int w, h;
  glfwGetFramebufferSize(impl->window, &w, &h);
  return uvec2{w, h};
}

uvec2 Application::getWindowSize() {
  int w, h;
...
void Application::handleWindowSizeChanged(uvec2 newSize)
{
	createSkiaSurface(newSize);
	resize();
}

void Application::createSkiaSurface(uvec2 size)
{
...
void Application::initialize(uvec2 initSize) {
