...
GLuint compile_and_attach_shader(GLuint program_obj, GLenum stage,
  ...
  if (!shader_obj) {
    ...
    std::cerr << "Compilation log follows:\n\n" << infoLog.str() << "\n\n";
    return 0;
  }
...
GLuint create_vertex_array_object(int num_attribs,
                                  const gl_vertex_attribute *attribs) {
  GLuint strides[gl_max_vertex_buffers] = {0};
  GLuint vertex_array_obj;
  gl::CreateVertexArrays(1, &vertex_array_obj);
  assert(num_attribs < g_gl_impl_limits.max_vertex_attributes);
  for (int attribindex = 0; attribindex < num_attribs; ++attribindex) {
...
gl_compute_pipeline gl_compute_pipeline::compile_from_file(
...
gl_compute_pipeline gl_compute_pipeline::compile_from_source(const char *src) {
  return gl_compute_pipeline{};
}

gl_graphics_pipeline gl_graphics_pipeline::compile_from_file(
    std::experimental::filesystem::path path_,
    std::initializer_list<pp_define> defines_, pipeline_stages stages_,
  ...
  pp.src =
      shader_source{shader_source::location::file, std::move(path_.string())};
  pp.defines = defines_;
  pp.stages = stages_;
...
void gl_compute_pipeline::do_load() {
  if (gl_program)
    return;

  if (src.loc == shader_source::location::file) {
    std::clog << "Loading compute pipeline: " << src.source_or_file_path
              << "\n";
    ...
    auto src2 = preprocess(file_path, def_c_str.data(), (int)def_c_str.size());
    gl_program = g_program_cache.query_cache_compute(
        src2.c_str(), program_cache::cache_action::load_and_cache,
...
// No caching yet
void gl_graphics_pipeline::do_load() {
  if (gl_program)
    return;
  ...
  if (src.loc == shader_source::location::file) {
    ...
    gl_vao = create_vertex_array_object(attribs.size(), attribs.data());
  } else {
    // TODO
  }
}

void gl_graphics_pipeline::bind() {
  do_load();
  gl::UseProgram(gl_program);
  gl::BindVertexArray(gl_vao);
  bind_draw_state(draw_state);
}

void gl_compute_pipeline::bind() {
  do_load();
  gl::UseProgram(gl_program);
}
