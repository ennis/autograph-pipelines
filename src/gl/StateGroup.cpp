					...
					for (int i = 0; i < 8; ++i) {
						if (sg.drawStates.blendStates[i].enabled) {
							...
							glBlendFuncSeparatei(i, sg.drawStates.blendStates[i].funcSrcRGB,
								sg.drawStates.blendStates[i].funcDstRGB, sg.drawStates.blendStates[i].funcSrcAlpha,
			...
			if (not_empty(sg.mask & StateGroupMask::DepthStencilState))
			{
				...
				if (sg.drawStates.depthStencilState.stencilEnable) {
					...
					glStencilFuncSeparate(
						sg.drawStates.depthStencilState.stencilFace, sg.drawStates.depthStencilState.stencilFunc,
						sg.drawStates.depthStencilState.stencilRef, sg.drawStates.depthStencilState.stencilMask);
