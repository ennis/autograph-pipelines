...
static GLFormatInfo glfmt_rgba32_uint{GL_RGBA32UI, GL_RGBA, GL_UNSIGNED_INT, 4,
                                      16};
static GLFormatInfo glfmt_depth32_float{GL_DEPTH_COMPONENT32F,
                                        GL_DEPTH_COMPONENT, GL_FLOAT, 1, 4};
static GLFormatInfo glfmt_argb_10_10_10_2_unorm{GL_RGB10_A2, GL_DEPTH_COMPONENT,
                                                GL_FLOAT, 1, 4};
...
Texture Texture::create2DMultisample(ImageFormat fmt, int w, int h,
  ...
  if (ms.count != 0) {
    ...
    glTextureStorage2DMultisample(tex_obj, ms.count, glfmt.internal_fmt, w, h,
                                  true);
  } else {
    glCreateTextures(GL_TEXTURE_2D, 1, &tex_obj);
