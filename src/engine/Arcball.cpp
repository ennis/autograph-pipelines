...
namespace ag
{
	...
	void Arcball::onMouseDrag(Transform& transform, const Camera& cam, int screenWidth, int screenHeight, int mouseX, int mouseY)
	{
		if (mouseX != mouseDownX || mouseY != mouseDownY) {
			...
			mat3 camera2object = glm::inverse(mat3(cam.viewMat) * mat3(transform.getMatrix()));
